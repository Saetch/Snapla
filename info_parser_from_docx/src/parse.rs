use crate::{ParsedContent, TagesBedarfInMg};
use docx_rust::document::{BodyContent, ParagraphContent, RunContent};
use docx_rust::DocxFile;
use fxhash::FxHashMap;

pub(super) fn read_parse_whacky_input_and_deserialize_docx_file(
    filepath: &str,
) -> (ParsedContent, TagesBedarfInMg) {
...
fn deserialize_content(content: &str) -> (ParsedContent, TagesBedarfInMg) {
    ...
    let mut rt = ParsedContent { lms: lms };
        ...
        if expect_mg_after_rel {
            if part != "mg" {
                ...
                if built_string.ends_with("mg") || built_string.ends_with("g") {
                    ...
                    current_lm_relative_weight = Some(
                        built_string
                            .replace(",", ".")
                            .replace("m", "")
                            .replace("g", "")
                    ...
                    if result.is_err()
                        && !(built_string.ends_with(".") || built_string.ends_with(","))
                    {
                ...
                println!(
                    "Weight in mg: {}",
                    current_lm_relative_weight.unwrap() as f64
                );
            }
        }
        let displayed_amount_in_mg = current_lm_relative_weight.unwrap();

        let mg_per_g = if displayed_amount_in_mg == 0.0 {
            0.0
        } else {
            displayed_amount_in_mg / current_lm_concentration_in_g as f64
        };
