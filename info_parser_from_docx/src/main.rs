...
fn main() {
    ...
    for file in information_folder {
        ...
        println!("{}", file_name);
        let parsed_content = read_parse_whacky_input_and_deserialize_docx_file(&format!(
            "information/{}",
            file_name
        ));
    ...
    for content in parsed_contents {
        for (k, value) in content.lms {
            ...
            if combined_info.contains_key(&key) {
                ...
                combined_info
                    .entry(key)
                    .and_modify(|map: &mut FxHashMap<String, f64>| {
                        map.extend(values.iter().map(|(k, v)| (k.clone(), *v)))
                    });
            } else {
